...
-- FIX #8: gender como tabla lookup en lugar de CHECK hardcodeado
CREATE TABLE gender (
...
CREATE TABLE person (
    ...
    -- FIX #8: FK a tabla gender en lugar de CHECK inline
    gender_id uuid REFERENCES gender(gender_id),
...
-- FIX #4: Columna is_current con constraint de unicidad parcial para tier activo
CREATE TABLE loyalty_account_tier (
    ...
    -- FIX #4: flag explícito para el tier vigente
...
-- FIX #4: Garantiza exactamente un tier activo por loyalty_account en todo momento
CREATE UNIQUE INDEX uq_loyalty_account_current_tier
    ON loyalty_account_tier (loyalty_account_id)
    WHERE is_current = true;

-- FIX #3: Tabla de tipos de transaccion de millas como lookup
CREATE TABLE miles_transaction_type (
    miles_transaction_type_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    type_code varchar(20) NOT NULL,
    type_name varchar(80) NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_miles_transaction_type_code UNIQUE (type_code),
    CONSTRAINT uq_miles_transaction_type_name UNIQUE (type_name)
);

-- FIX #3: miles_transaction con FK tipada al origen (ticket o ajuste manual)
CREATE TABLE miles_transaction (
    ...
    -- FIX #3: FK opcional al ticket que origina la acumulacion; NULL para ADJUST
    ticket_id uuid,
    miles_delta integer NOT NULL,
    occurred_at timestamptz NOT NULL,
    -- FIX #3: reference_code se mantiene para integraciones externas, pero ya no es el unico trazador
...
-- FK a ticket se agrega despues de crear la tabla ticket (ver seccion de ticketing)
...
CREATE TABLE airport (
    ...
    -- FIX #15: FK desnormalizada a country para evitar 5 joins en consultas frecuentes
    country_id uuid NOT NULL REFERENCES country(country_id),
...
-- FIX #14: maintenance_event_status como tabla lookup (consistente con el resto del esquema)
...
-- FIX #14: usa FK a tabla lookup en lugar de CHECK hardcodeado
...
CREATE TABLE flight_status (
    ...
    status_code varchar(20) NOT NULL,
    status_name varchar(80) NOT NULL,
    -- FIX #9: flag para saber si este status implica vuelo completado
...
-- FIX #3: ahora que ticket existe, agregamos la FK desde miles_transaction
ALTER TABLE miles_transaction
    ADD CONSTRAINT fk_miles_transaction_ticket
...
-- FIX #2: funcion que valida que el flight_segment pertenece al mismo vuelo
--         que la reservation del ticket (evita tickets en vuelos no relacionados)
CREATE OR REPLACE FUNCTION fn_check_ticket_segment_reservation()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
    v_airline_id uuid;
    v_flight_airline_id uuid;
BEGIN
    -- Obtener airline de la reserva a traves del ticket
    SELECT a.airline_id INTO v_airline_id
    FROM ticket t
    ...
    JOIN airline a ON a.airline_id = c.airline_id
    WHERE t.ticket_id = NEW.ticket_id;

    -- Obtener airline del vuelo del segmento
    SELECT f.airline_id INTO v_flight_airline_id
    FROM flight_segment fs
    JOIN flight f ON f.flight_id = fs.flight_id
    WHERE fs.flight_segment_id = NEW.flight_segment_id;

    -- Si la reserva no tiene customer (agencia), omitir validacion
    IF v_airline_id IS NOT NULL AND v_flight_airline_id IS NOT NULL THEN
        IF v_airline_id <> v_flight_airline_id THEN
            RAISE EXCEPTION 'El segmento de vuelo (airline_id=%) no corresponde a la aerolínea de la reserva (airline_id=%)',
                v_flight_airline_id, v_airline_id;
        END IF;
    END IF;
...
CREATE TRIGGER trg_ticket_segment_reservation_check
    ...
    FOR EACH ROW EXECUTE FUNCTION fn_check_ticket_segment_reservation();

-- FIX #1: funcion que valida que el asiento pertenece al avion del segmento
CREATE OR REPLACE FUNCTION fn_check_seat_assignment_aircraft()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE
    v_seat_aircraft_id uuid;
    v_segment_aircraft_id uuid;
BEGIN
    -- Avion del asiento
    SELECT ac.aircraft_id INTO v_seat_aircraft_id
    FROM aircraft_seat as2
    JOIN aircraft_cabin ac ON ac.aircraft_cabin_id = as2.aircraft_cabin_id
    WHERE as2.aircraft_seat_id = NEW.aircraft_seat_id;

    -- Avion del segmento de vuelo
    SELECT f.aircraft_id INTO v_segment_aircraft_id
    FROM flight_segment fs
    JOIN flight f ON f.flight_id = fs.flight_id
    WHERE fs.flight_segment_id = NEW.flight_segment_id;

    IF v_seat_aircraft_id IS DISTINCT FROM v_segment_aircraft_id THEN
        RAISE EXCEPTION 'El asiento (aircraft_id=%) no pertenece al avión del segmento (aircraft_id=%)',
            v_seat_aircraft_id, v_segment_aircraft_id;
    END IF;
...
CREATE TRIGGER trg_seat_assignment_aircraft_check
    ...
    FOR EACH ROW EXECUTE FUNCTION fn_check_seat_assignment_aircraft();
...
-- FIX #16: refund referencia la transaccion de captura especifica, no solo el payment padre
CREATE TABLE refund (
    refund_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    payment_id uuid NOT NULL REFERENCES payment(payment_id),
    -- FIX #16: FK a la transaccion de captura que origina el reembolso
    source_payment_transaction_id uuid REFERENCES payment_transaction(payment_transaction_id),
...
-- FIX #10: exchange_rate con tipo de tasa y fuente
CREATE TABLE exchange_rate_source (
    exchange_rate_source_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    source_code varchar(20) NOT NULL,
    source_name varchar(80) NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_exchange_rate_source_code UNIQUE (source_code),
    CONSTRAINT uq_exchange_rate_source_name UNIQUE (source_name)
);
...
CREATE TABLE exchange_rate (
    ...
    -- FIX #10: fuente de la tasa (BCE, banco emisor, proveedor externo, etc.)
    exchange_rate_source_id uuid REFERENCES exchange_rate_source(exchange_rate_source_id),
    effective_date date NOT NULL,
    -- FIX #10: tipo de tasa: MID (mid-market), BUY (compra), SELL (venta)
    rate_type varchar(10) NOT NULL DEFAULT 'MID',
    rate_value numeric(18, 8) NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_exchange_rate UNIQUE (from_currency_id, to_currency_id, effective_date, rate_type),
    ...
    CONSTRAINT ck_exchange_rate_type CHECK (rate_type IN ('MID', 'BUY', 'SELL'))
);
...
-- FIX #7: invoice_line_source_type para identificar el origen del concepto facturado
CREATE TABLE invoice_line_source_type (
    invoice_line_source_type_id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    source_type_code varchar(20) NOT NULL,
    source_type_name varchar(80) NOT NULL,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_invoice_line_source_type_code UNIQUE (source_type_code),
    CONSTRAINT uq_invoice_line_source_type_name UNIQUE (source_type_name)
);

-- FIX #7: invoice_line con FK trazable al ticket o baggage que origina el concepto
CREATE TABLE invoice_line (
    ...
    -- FIX #7: tipo de origen para saber a qué tabla apunta source_entity_id
    invoice_line_source_type_id uuid REFERENCES invoice_line_source_type(invoice_line_source_type_id),
    -- FIX #7: ID del ticket, baggage u otro origen (polimórfico controlado por source_type)
    ...
    -- FIX #7: source_entity_id solo puede estar presente si source_type esta definido
    CONSTRAINT ck_invoice_line_source CHECK (
        (invoice_line_source_type_id IS NULL AND source_entity_id IS NULL)
        OR (invoice_line_source_type_id IS NOT NULL AND source_entity_id IS NOT NULL)
    )
);

-- ============================================
-- FIX #12: TRIGGER updated_at PARA TODAS LAS TABLAS
-- ============================================
...
-- Macro para aplicar el trigger a todas las tablas con updated_at
DO $$
DECLARE
    t text;
BEGIN
...
COMMENT ON TABLE seat_assignment IS 'Asignacion de asiento normalizada por ticket_segment con control de unicidad por segmento y asiento. Trigger trg_seat_assignment_aircraft_check garantiza que el asiento pertenece al avion del segmento.';
COMMENT ON TABLE loyalty_account_tier IS 'Historial de asignacion de nivel. El indice parcial uq_loyalty_account_current_tier garantiza exactamente un tier activo (is_current=true) por cuenta.';
COMMENT ON TABLE invoice_line IS 'Detalle facturable sin totales derivados persistidos (3FN). invoice_line_source_type_id + source_entity_id proveen trazabilidad al origen del concepto facturado.';
COMMENT ON TABLE miles_transaction IS 'Transacciones de millas con FK opcional a ticket para trazabilidad auditada. miles_transaction_type_id reemplaza el CHECK hardcodeado original.';
COMMENT ON TABLE exchange_rate IS 'Tasas de cambio con tipo (MID/BUY/SELL) y fuente para soporte de billing multi-moneda auditado.';
COMMENT ON TRIGGER trg_set_updated_at ON time_zone IS 'Trigger generado automaticamente para mantener updated_at correcto en UPDATE.';
...
-- FIX #15: indice sobre la FK desnormalizada de pais en airport
CREATE INDEX idx_airport_country_id ON airport(country_id);
...
-- FIX #16: indice sobre la nueva FK a payment_transaction
CREATE INDEX idx_refund_source_transaction_id ON refund(source_payment_transaction_id);
...
-- FIX #7: indice sobre source_entity_id para busquedas por origen de linea
CREATE INDEX idx_invoice_line_source_entity_id ON invoice_line(source_entity_id) WHERE source_entity_id IS NOT NULL;
